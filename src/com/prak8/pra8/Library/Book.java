/* TODO: Lengkapi class Book dengan validasi ISBN dan exception handling
   ...
   - boolean isValidISBN10(String isbn)
     * ISBN-10 checksum algorithm:
       - Sum = (digit1 x 10) + (digit2 x 9) + ... + (digit9 x 2) + check_digit
   ...
   - boolean isValidISBN13(String isbn)
     * ISBN-13 checksum algorithm:
       - Sum = digit1 + (digit2 x 3) + digit3 + (digit4 x 3) + ... + digit13
       - Digit ganjil (index 0,2,4...) x 1, digit genap (index 1,3,5...) x 3
       - Valid jika Sum % 10 == 0
...
public class Book {
    ...
    public Book(String title, String author, String isbn, int publicationYear, int copies) throws InvalidBookException {
        ...
        if (title == null || title.trim().isEmpty()) {
            throw new InvalidBookException("Title cannot be empty", "", "title");
        }

        // Validate author
        if (author == null || author.trim().isEmpty()) {
            throw new InvalidBookException("Author cannot be empty", title, "author");
        }

        String[] authorNames = author.trim().split("\\s+");
        if (authorNames.length < 2) {
            throw new InvalidBookException("Author must have at least two names", title, "author", author);
        }
        ...
        if (publicationYear < 1000 || publicationYear > currentYear) {
            throw new InvalidBookException(
                ...
                title, "publicationYear", String.valueOf(publicationYear)
            );
        }
        ...
        if (copies < 0) {
            throw new InvalidBookException(
                ...
                title, "copies", String.valueOf(copies)
            );
        }
    ...
    public void returnBook() throws InvalidBookException {
        if (availableCopies >= copies) {
            throw new InvalidBookException(
                "Cannot return more books than total copies",
                title,
                "availableCopies"
            );
        }
