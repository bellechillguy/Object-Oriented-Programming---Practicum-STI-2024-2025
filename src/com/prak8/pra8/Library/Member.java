...
public class Member {
    ...
    public void borrowBook(String bookTitle) throws LibraryException {
        ...
        // 2. Cek belum meminjam buku ini
        if (hasBorrowedBook(bookTitle)) {
            throw new LibraryException(
                String.format("Anggota %s sudah meminjam buku '%s'.", name, bookTitle),
                "ALREADY_BORROWED"
            );
        }

        // 3. Cek belum mencapai limit
        if (getCurrentBorrowedCount() >= maxBorrowLimit) {
            throw new LibraryException(
                String.format("Anggota %s telah mencapai batas peminjaman (%d/%d).", 
                    name, getCurrentBorrowedCount(), maxBorrowLimit),
                "LIMIT_EXCEEDED"
            );
        }
    ...
    public void returnBook(String bookTitle) throws LibraryException {
        if (!hasBorrowedBook(bookTitle)) {
            throw new LibraryException(
                String.format("Anggota %s tidak meminjam buku '%s'.", name, bookTitle),
                "NOT_BORROWED"
            );
        }
    ...
    public String toString() {
        return String.format("%s (ID: %s) - Borrowed: %d/%d",
            name, memberId, getCurrentBorrowedCount(), maxBorrowLimit);
    }
}
